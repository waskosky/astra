...
if ( ! class_exists( 'Astra_Ext_Widgets_Helper' ) ) :

	/**
	 * Astra_Ext_Widgets_Helper
	 *
	 * @since 1.6.0
	 */
	class Astra_Ext_Widgets_Helper {
		...
		function backend_load_astra_fonts() {
			...
			/* Define Variables */
			$uri  = ASTRA_THEME_URI . 'assets/css/';
			$path = ASTRA_THEME_DIR . 'assets/css/';
			$rtl  = '';
			...
			wp_enqueue_style( 'astra-font-style', $css_uri . 'font-style' . $file_prefix . '.css', null, ASTRA_THEME_VERSION, 'all' );
	 ...
	 */
	Astra_Ext_Widgets_Helper::get_instance();
...
if ( ! function_exists( 'astra_generate_widget_fields' ) ) :
	...
	function astra_generate_widget_fields( $self, $fields = array(), $repeater_id = '' ) {
		Astra_Ext_Widgets_Helper::get_instance()->generate( $self, $fields, $repeater_id );
	}
endif;
